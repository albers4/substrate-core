...
use substrate_core_spec::array::{ArrayLike, memory_order::MemoryOrder, ops::AccessOps};
...
impl<'a> ArrayView<'a, f64> {
    ...
    pub fn matmul_scalar(
        ...
        for i in 0..m {
            for p in 0..k {
                let a_val = unsafe { self.get_unchecked(&[i, p]) };
                for j in 0..n {
                    result[i * n + j] += a_val * unsafe { other.get_unchecked(&[p, j]) };
                }
            }
        }
